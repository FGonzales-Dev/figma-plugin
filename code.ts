...
figma.ui.onmessage = async (msg: {
  type: string;
  mobileUrlTextbox: string;
  desktopUrlTextbox: string;
}) => {
  ...
  if (msg.type === "create-rectangles") {
    if (
      !msg.desktopUrlTextbox.includes("figma.com/file") &&
      ...
      while (!saved) {
        ...
        try {
          const response = await fetch(
            `https://firestore.googleapis.com/v1/projects/figmawebapp/databases/(default)/documents/pluginFolio/${randomId}`,
            {
              method: "PATCH",
          ...
          if (response.ok) {
            figma.notify(
              `Data successfullyxxxxssss sent to Firestore with ID: ${randomId}`
            );
            ...
            if (
              ...
              figma.notify(
                `Document ID ${randomId} already exists. Generating a new ID...`
              );
              // Continue the loop to try with a new ID
            } else {
              figma.notify(
                `Failed to send data to Firestore: ${
    ...
    if (url !== "") {
      ...
      if (newUrl.includes("scaling=contain")) {
        if (!newUrl.includes("content-scaling=responsive")) {
          modifiedUrl = newUrl.replace(
            new RegExp("scaling=contain", "g"),
        ...
        if (!newUrl.includes("scaling=scale-down-width")) {
          modifiedUrl = newUrl.replace(
            new RegExp("scaling=scale-down", "g"),
            "scaling=scale-down-width"
          );
          newUrl = modifiedUrl;
        }
